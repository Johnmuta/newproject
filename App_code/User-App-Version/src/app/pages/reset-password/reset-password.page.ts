...
export class ResetPasswordPage implements OnInit {
  ...
  sendOTPonPhone(form: NgForm) {
    ...
    if (form.valid) {
      this.util.show();
      if (this.util.smsGateway == '2' && this.util.isWebOrPWA == false) { // Firebase OTP ON PHONE
      ...
      } else if (this.util.smsGateway == '2' && this.util.isWebOrPWA == true) { // Firebase OTP ON WEB PWA
        console.log('pwa window');
        this.api.signInWithPhoneNumber(this.recaptchaVerifier, this.loginWithPhoneOTP.country_code + this.loginWithPhoneOTP.mobile).then(
          success => {
            this.util.hide();
  ...
  async openVerificationModal(id, to, obj) {
    this.otpId = id;
  ...
  async OtpVerification() {
    const alert = await this.alertController.create({
      header: this.util.translate('Enter OTP'),
      backdropDismiss: false,
      inputs: [
        {
          name: 'otp',
          type: 'text',
          placeholder: this.util.translate('Enter your otp'),
        }
      ],
      buttons: [{
        text: this.util.translate('Enter'),
        handler: (res) => {
